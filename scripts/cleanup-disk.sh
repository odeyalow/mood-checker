#!/bin/bash
# Auto-cleanup script: removes old temporary files and reduces disk pressure.
...
# Aggressively clean old face images if disk still high.
# At >90% keep 7 days, at >80% keep 30 days.
DISK_USAGE_AFTER=$(df / | awk 'NR==2 {print $5}' | sed 's/%//')
if [ "$DISK_USAGE_AFTER" -gt 90 ]; then
  log "Disk still very high ($DISK_USAGE_AFTER%). Cleaning face images older than 7 days..."
  find "$MOOD_CHECKER_DIR/public/_faces" -type f -mtime +7 -delete 2>/dev/null || true
  FACES_COUNT=$(find "$MOOD_CHECKER_DIR/public/_faces" -type f 2>/dev/null | wc -l)
  log "Remaining face images: $FACES_COUNT"
elif [ "$DISK_USAGE_AFTER" -gt 80 ]; then
  log "Disk still high ($DISK_USAGE_AFTER%). Cleaning old face images..."
  find "$MOOD_CHECKER_DIR/public/_faces" -type f -mtime +30 -delete 2>/dev/null || true
  FACES_COUNT=$(find "$MOOD_CHECKER_DIR/public/_faces" -type f 2>/dev/null | wc -l)
  log "Remaining face images: $FACES_COUNT"
fi
...
# Never restart PM2 processes from cleanup; it does not free disk space.
if [ "$DISK_USAGE_FINAL" -gt 95 ]; then
  ...
  log "WARN: Disk usage ${DISK_USAGE_FINAL}% exceeds threshold ${DISK_THRESHOLD}%. Monitor required."
else
  log "OK: Disk cleanup successful. Disk usage: ${DISK_USAGE_FINAL}%"
fi
